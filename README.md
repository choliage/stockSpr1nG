...
StockSpr1nG 是一個用於抓取台股新聞、掃描關鍵字、下載 MOPS 財報，以及生成新聞與財報整合分析報表的 Python 專案。

## 功能概覽

- 從 UDN 股市新聞頁面爬取最新股票新聞，並儲存為 TXT 檔案
- 扫描已抓取新聞中的關鍵字，按月輸出 Excel 檔案
- 下載指定公司代號的 MOPS 財報 PDF
- 匯整新聞與財報資訊，生成分析 Excel 報表

## 目錄結構

- `main.py`：主程式入口，提供互動式選單
- `scraper.py`：爬取 UDN 最新股票新聞並儲存為 `.txt`
- `keyword_scanner.py`：掃描新聞檔案並匯出月度關鍵字 Excel
- `financial_report_scraper.py`：下載 MOPS 財報 PDF
- `news_financial_analyzer.py`：生成整合分析報表
- `company_names.json`：公司名稱與代號對照表
- `company_codes.json`：公司代號清單，可用於財報下載或其它流程
- `mongodb_helper.py`：MongoDB 相關協助函式
- `log.py`：日誌設定
...
若你要新增關鍵字，請在 `keyword_scanner.py` 的 `KEYWORDS` 清單中調整。

若你要新增公司名稱對照，請編輯 `company_names.json`。

---

如果你希望，我也可以再幫你補上專案執行範例、資料夾結構圖，以及常見錯誤排除說明。